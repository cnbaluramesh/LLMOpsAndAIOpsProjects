...
gcloud storage cp files_to_upload.txt gs://${BUCKET_NAME}

# Upload multiple files using cp command 
gsutil cp *.txt gs://${BUCKET_NAME}
gcloud storage cp *.txt gs://${BUCKET_NAME}
...
gcloud storage cp --recursive --threads=8 gs://${BUCKET_NAME} .




gcloud storage cp *.txt gs://${BUCKET_NAME} --recursive